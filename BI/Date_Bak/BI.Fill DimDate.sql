...
BEGIN
	...
	BEGIN		
		...
		INSERT INTO [Dim_Date](
			...
			[time_Key_Year_Week_Native],
			[time_Key_Year_Month_Native],
			[time_Key_Year_Half_Native],
			[time_Key_Year_Season_Native],
			[time_Key_Year_Native],--[time_Key_Year_Month_Day_Hour],
			[time_Key_Year_Month_Day],
			[time_Key_Year_Month],
			[time_Key_Year_Half],
			[time_Key_Year_Season],
			[time_Key_Year],--[time_Title_Year_Month_Day_Hour],
			[time_Title_Year_Month_Day],
			[time_Title_Year_Week],
			[time_Title_Year_Month],
			[time_Title_Year_Half],
			[time_Title_Year_Season],
			[time_Title_Year],
			[time_Month_Native],
			[time_Month_Native_2CHAR],
			[time_Day_Native],
			[time_Day_Native_2CHAR],--[time_Hour],[time_Hour_2CHAR],
			[time_Date_Name],
			[time_Year],
			[time_Month_Of_Year],
			[time_Month_Of_Year_2CHAR],
			[time_Month_Of_Season],
			[time_Month_Name],
			[time_Day_Of_Year],
			[time_Day_Of_Month],
			[time_Day_Of_Month_2CHAR],
			[time_Day_Of_Week],
			[time_Day_Of_Week_Name],
			[time_Week_Of_Year],
			[time_Season_Of_Year],
			[time_Season_Name],
			[time_Half_Of_Year],
		...
		(
			...
			@mYear  * 10000		+ @mMonth * 100		  + @mDay,  --time_Key_Year_Month_Day_Native
			@mYear  * 100		+ @mWeekOfYear, -- time_Key_Year_Week_Native
			@mYear  * 100		+ @mMonth,  -- time_Key_Year_Month_Native
			@mYear  * 10		+ CEILING(CAST(@mMonth AS Money) / 6 ),  -- time_Key_Year_Half_Native
			@mYear  * 10		+ CEILING(CAST(@mMonth AS Money) / 3 ) , -- time_Key_Year_Season_Native
			@mYear, -- time_Key_Year_Native
			@sYear  * 10000	+ @sMonth * 100 + @sDay, -- time_Key_Year_Month_Day
			@sYear  * 100		+ @sMonth, -- time_Key_Year_Month
			--@sYear  * 100		+ @sWeekOfYear,
			@sYear  * 10		+ CEILING(CAST(@sMonth AS Money) / 6 ),  --time_Key_Year_Half
			@sYear  * 10		+ CEILING(CAST(@sMonth AS Money) / 2 ), --time_Key_Year_Season
			@sYear,	--time_Key_Year
			--@sYear,
			--N'ساعت ' + CAST(@Hour AS NVARCHAR(2))+ N' ' + N'روز' + N' ' + @sDate,
			@sDate, --time_Title_Year_Month_Day
			N'هفته ' + CAST(@sDayOfWeek AS NVARCHAR(2))+ N' سال ' + @sYearCH, --time_Title_Year_Week
			@sMonthName + N' ' + @sYearCH,  --time_Title_Year_Month 
			@sHalfYearName + N' ' + @sYearCH,  --time_Title_Year_Half
			@sSeasonName + N' ' + @sYearCH,  --time_Title_Year_Season
			N'سال ' + @sYearCH,  --time_Title_Year
			@sMonth,  --time_Month_Native
			@sMonthCH,  --time_Month_Native_2CHAR
			@sDay,  --time_Day_Native
			@sDayCH,  --time_Day_Native_2CHAR
			--@Hour,
			--@HourCH, 
			N'روز' + CAST(@sDay AS NVARCHAR(2)) + N' ' + @sMonthName + N' ' + @sYearCH, --time_Date_Name
			@sYear,  --time_Year
			@sMonth,  --time_Month_Of_Year
			@sMonthCH, --time_Month_Of_Year_2CHAR
			CASE WHEN @sMonth % 3 = 0 THEN 3 ELSE @sMonth % 3 END,  --time_Month_Of_Season
			@sMonthName, --time_Month_Name
			@sDayOfYear,  --time_Day_Of_Year
			@sDay,  --time_Day_Of_Month
			@sDayCH,  --time_Day_Of_Month_2CHAR
			@sDayOfWeek,   --time_Day_Of_Week
			@sDayOfWeekName,  --time_Day_Of_Week_Name
			@sWeekOfYear,  -- time_Week_Of_Year
			CEILING(CAST(@sMonth AS Money) / 3),  --time_Season_Of_Year
			@sSeasonName,  --time_Season_Name
			CEILING(CAST(@sMonth AS Money) / 6 ),  --time_Half_Of_Year
			@sHalfYearName,  --time_Half_Of_Year_Name
			@mDate  --FullDate
		)
		SET @Hour = @Hour + 1
	END
...
END
Go
Select time_Year, time_Title_Year_Month_Day, time_Season_Name, time_Season_Of_Year, time_Title_Year_Season, time_key_year,
	time_Month_Name, *
from Dim_Date
Order By FullDate
Go
